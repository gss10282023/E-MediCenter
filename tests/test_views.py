...
class RenderingTest(TestCase):
    ...
    def test_admin_doctors_dashboard(self):
        response = self.client.get(reverse('admin_doctors'))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "Dashboard_Admin_doctors.html")
...
class LoginViewTestCase(TestCase):
    ...
    def add_session_to_request(self, request):
        middleware = SessionMiddleware(lambda req: None)  # Provide a no-op get_response
        middleware.process_request(request)
        request.session.save()
        return request
...
class AdminGetOrderTests(TestCase):
    ...
    def test_get_all_orders(self):
        response = self.client.get(reverse('api_caregiver_orders'))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json()), 2)

    def test_get_all_doctors(self):
        response = self.client.get(reverse('api_doctors'))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json()), 2)

    def test_get_featured_doctors(self):
        response = self.client.get(reverse('api_featured_doctors'))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json()), 2)  

    def test_get_recent_GP_orders(self):
        response = self.client.get(reverse('api_recent_gp_orders'))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json()), 2)

    def test_get_all_GP_orders(self):
        response = self.client.get(reverse('api_gp_orders'))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json()), 2)

    def test_get_recent_orders(self):
        response = self.client.get(reverse('api_recent_caregiver_orders'))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.json()), 2)
